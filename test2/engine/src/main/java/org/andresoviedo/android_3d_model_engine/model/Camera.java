...
//		this(0, 0, 6, 0, 0, 0, 0, 1, 0);
	...
	public synchronized void animate(){
		...
		if (method.equals("translate")){/**ban đầu */
		...
		}else if (method.equals("translatee")){	/**lúc sau*/
			float dX = (Float) lastAction[1];
			float dY = (Float) lastAction[2];
			translateCameraImpl1(dX*animationCounter/100, dY*animationCounter/100);}
		animationCounter--;
	}

	public synchronized void MoveCameraZ(float direction){/**zoom*/
		if (direction == 0) return;
		MoveCameraZImpl(direction);
		lastAction = new Object[]{"zoom",direction};
	}
	private void MoveCameraZImpl(float direction) {/**zoom*/
		...
		/***********************************************/
//        float x = xPos + xLookDirection * direction*2f;
//        float y = yPos + yLookDirection * direction*2f;
//        float z = zPos + zLookDirection * direction*2f;
     ...
     */
	/** * Kiểm tra xem vị trí được chỉ định là "tường" bên ngoài phòng hay ở chính giữa phòng.
	 * * Vị trí @param x x
	 * * Vị trí @param y y
	 * * Vị trí @param z z
	 * * @return true nếu vị trí được chỉ định là bên ngoài "tường" của phòng hoặc ở chính giữa phòng
	 * */
	private boolean isOutOfBounds(float x, float y, float z) {/**không liên quan di chuyển và zoom*/
	 ...
	 * /** * Tịnh tiến là chuyển động tạo nên Trái đất quanh Mặt trời.
	 *
	 * So in this context, translating the camera means moving the camera around the Zero (0,0,0)
	 * Vì vậy, trong bối cảnh này, dịch máy ảnh có nghĩa là di chuyển máy ảnh xung quanh Không (0,0,0)
	 * *
	 *
	 * This implementation makes uses of 3D Vectors Algebra.
	 * * Việc triển khai này sử dụng Đại số vectơ 3D. *
	 *
	 * The idea behind this implementation is to translate the 2D user vectors (the line in the
	 * screen) with the 3D equivalents.
	 *  * Ý tưởng đằng sau việc triển khai này là dịch các vectơ người dùng 2D
	 *  (đường trong màn hình *) với các đối tượng 3D tương đương. *
	 *
	 * In order to to that, we need to calculate the Right and Arriba vectors so we have a match
	 ** Để làm được điều đó, chúng ta cần tính toán các vectơ Right và Arriba để chúng tôi khớp
	 *
	 *  for user 2D vector.
	 *   * cho vectơ 2D của người dùng. *
	 *
	 * @param dX the X component of the user 2D vector, that is, a value between [-1,1]
	 **@param dX thành phần X của vectơ 2D người dùng, nghĩa là giá trị trong khoảng [-1,1]
	 *
	 *@param dY the Y component of the user 2D vector, that is, a value between [-1,1]
	 ** @param dY thành phần Y của vectơ 2D người dùng, nghĩa là giá trị trong khoảng [-1,1 ]
	 */
	/**ban đầu */
	public synchronized void translateCamera(float dX, float dY) {
		//Log.v("Camera","translate:"+dX+","+dY);
		if (dX == 0 && dY == 0) return;
		translateCameraImpl(dX, 0);
		lastAction = new Object[]{"translate",dX, dY};
	}
	/**lúc sau*/
	public synchronized void translateCamera1(float dX, float dY) {
		//Log.v("Camera","translate:"+dX+","+dY);
		if (dX == 0 && dY == 0) return;
		translateCameraImpl1(dX, dY);
		lastAction = new Object[]{"translatee",dX, dY};
	}
/**ban đầu */
...
//		createRotationMatrixAroundVector(buffer, 24, dX, xArriba, yArriba, zArriba);
		createRotationMatrixAroundVector(buffer, 24, dX, xArriba, yArriba, 0);
	}
//	else{
...
//	}
	...
	/**lúc sau*/
	private void translateCameraImpl1(float dX, float dY) {
		...
		if (Math.abs(dX) > Math.abs(dY)) {
			...
			float xRight = (yLook * 0) - (zLook * 1);
			float yRight = (zLook * 0) - (xLook * 1);
			float zRight = (xLook * 1) - (yLook * 0);
			vlen = Matrix.length(xRight, yRight, zRight);
			xRight /= vlen;
			yRight /= vlen;
			zRight /= vlen;
			...
			/***********************************************/
//			float xTranslation = dX * xRight*4f;
//			float zTranslation = dX * zRight*4f;
			float xTranslation = dX * xRight;
			float zTranslation = dX * zRight;
			...
			/***********************************************/
//			float yTranslation = dY * yUp*4f;
			float yTranslation = dY * yUp;
	...
	private static void createRotationMatrixAroundVector(float[] matrix, int offset, float angle, float x, float y,/**giúp camera quay quanh vật 3d*/
                                                         float z) {/**có ảnh hưởng đến di chuyển*/
	...
	private static void multiplyMMV(float[] result, int retOffset, float[] matrix, int matOffet, float[] vector4Matrix,
                                    int vecOffset) {/**có ảnh hưởng đến di chuyển*/
	...
	/**Xoay quanh trục y**/
	public synchronized void Rotate1(float rotViewerZ) {
		if (rotViewerZ == 0) return;
		RotateImpl1(rotViewerZ);
		lastAction = new Object[]{"rotate",rotViewerZ};
	}

	private void RotateImpl1(float rotViewerY) {
		if (Float.isNaN(rotViewerY)) {
			Log.w("Rot", "NaN");
			return;
		}
		float xLook = xView - xPos;
		...
		// Tạo ma trận quay xung quanh trục y
		createRotationMatrixAroundVector(buffer, 24, rotViewerY, 0, 1, 0); // Thay đổi tham số xLook, yLook, zLook thành 0, 1, 0
